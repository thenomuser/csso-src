...
int Option_ActivityModifier( s_sequence_t *psequence )
{
	...
	if (token[0] == '{')
	{
		while ( TokenAvailable() )
		{
			...
			strlwr(token);
			strcpy( psequence->activitymodifier[ psequence->numactivitymodifiers++ ].name, token );
		}
	}
	else
	{
	...
	while (1)
	{
		...
		*/
		...
		else if (stricmp("rootdriver", token) == 0)
		{
			...
			strcpy( pseq->rootDriverBoneName, token );
		}
...
void Cmd_BoneAlwaysSetup( )
{
	...
	// bone name
	GetToken (false);
	strcpy( g_BoneAlwaysSetup[nIndex].bonename, token );
}
