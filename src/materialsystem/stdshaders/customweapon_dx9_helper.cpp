...
void InitParamsCustomWeapon( CBaseVSShader *pShader, IMaterialVar** params, const char *pMaterialName, CustomWeaponVars_t &info )
{
	...
	if ( ( g_pHardwareConfig->SupportsShaderModel_3_0() ) && ( info.m_nPreview != -1 ) && ( params[info.m_nPreview]->GetIntValue() == 1 ) )
	{
	...
	CBasePerMaterialContextData **pContextDataPtr )
{
	...
	bool bPreview = ( g_pHardwareConfig->SupportsShaderModel_3_0() ) && ( info.m_nPreview != -1 ) && ( params[info.m_nPreview]->GetIntValue() == 1 );
	...
	if ( ( pShader->IsSnapshotting() && pShaderShadow ) || (! pContextData ) || ( pContextData->m_bMaterialVarsChanged ) )
	{
		...
		if ( pShaderAPI && ( (! pContextData ) || ( pContextData->m_bMaterialVarsChanged ) ) )
		{
			...
			if( bAOTexture ) 
			{
				pContextData->m_SemiStaticCmdsOut.BindTexture( pShader, SHADER_SAMPLER0, info.m_nAOTexture, -1 );
			}

			// Scratches
			if( bWearTexture )
			{
				pContextData->m_SemiStaticCmdsOut.BindTexture( pShader, SHADER_SAMPLER1, info.m_nWearTexture, -1 );
			}

			// Exponent
			if( bExpTexture )
			{
				pContextData->m_SemiStaticCmdsOut.BindTexture( pShader, SHADER_SAMPLER2, info.m_nExpTexture, -1 );
			}

			// Base
			if ( bBaseTexture )
			{
				pContextData->m_SemiStaticCmdsOut.BindTexture( pShader, SHADER_SAMPLER3, info.m_nBaseTexture, -1 );
			}

			// Masks
			if( bMasksTexture )
			{
				pContextData->m_SemiStaticCmdsOut.BindTexture( pShader, SHADER_SAMPLER4, info.m_nMasksTexture, -1 );
			}

			//Grunge
			if( bGrungeTexture )
			{
				pContextData->m_SemiStaticCmdsOut.BindTexture( pShader, SHADER_SAMPLER5, info.m_nGrungeTexture, -1 );
			}

			// Object Space Normals
			if( bSurfaceTexture )
			{
				pContextData->m_SemiStaticCmdsOut.BindTexture( pShader, SHADER_SAMPLER6, info.m_nSurfaceTexture, -1 );
			}

			// Object Space Position
			if( bPosTexture )
			{
				pContextData->m_SemiStaticCmdsOut.BindTexture( pShader, SHADER_SAMPLER7, info.m_nPosTexture, -1 );
			}

			// Paint pattern
			if( bPaintTexture )
			{
				pContextData->m_SemiStaticCmdsOut.BindTexture( pShader, SHADER_SAMPLER8, info.m_nPaintTexture, -1 );
			}

			if ( bPreview )
			{
				pContextData->m_SemiStaticCmdsOut.BindStandardTexture( SHADER_SAMPLER9, TEXTURE_NORMALIZATION_CUBEMAP_SIGNED );
			}
