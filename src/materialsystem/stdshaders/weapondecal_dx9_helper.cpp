	...
	CBasePerMaterialContextData **pContextDataPtr )
{
	...
	if ( ( pShader->IsSnapshotting() && pShaderShadow ) || (! pContextData ) || ( pContextData->m_bMaterialVarsChanged ) )
	{
		...
		if ( pShaderAPI && ( (! pContextData ) || ( pContextData->m_bMaterialVarsChanged ) ) )
		{
			...
			if ( bBaseTexture )
			{
				pContextData->m_SemiStaticCmdsOut.BindTexture( pShader, SHADER_SAMPLER0, info.m_nBaseTexture, -1 );
			}
			if( bAOTexture && !bThirdPerson ) 
			{
				pContextData->m_SemiStaticCmdsOut.BindTexture( pShader, SHADER_SAMPLER1, info.m_nAOTexture, -1 );
			}
			if( bExpTexture )
			{
			...
			{
				pContextData->m_SemiStaticCmdsOut.BindTexture( pShader, SHADER_SAMPLER6, info.m_nNormalMap, -1 );
			}
			if( bEnvmapTexture )
			{
				pContextData->m_SemiStaticCmdsOut.BindTexture( pShader, SHADER_SAMPLER3, info.m_nEnvmapTexture, -1 );
			}
			if( bWearTexture && !bThirdPerson )
			{
				pContextData->m_SemiStaticCmdsOut.BindTexture( pShader, SHADER_SAMPLER4, info.m_nWearTexture, -1 );
			}
			if( bGrungeTexture && !bThirdPerson )
			{
				pContextData->m_SemiStaticCmdsOut.BindTexture( pShader, SHADER_SAMPLER9, info.m_nGrungeTexture, -1 );
			}
			if ( bHologramSpectrum && ( nDecalStyle != 4 ) && ( nDecalStyle != 5 ) )
			{
				pContextData->m_SemiStaticCmdsOut.BindTexture( pShader, SHADER_SAMPLER6, info.m_nHologramSpectrum, -1 );
			}
			if( bHologramMask )
			{
				pContextData->m_SemiStaticCmdsOut.BindTexture( pShader, SHADER_SAMPLER7, info.m_nHologramMask, -1 );
			}
			if ( bAnisoDirTexture )
			{
				pContextData->m_SemiStaticCmdsOut.BindTexture( pShader, SHADER_SAMPLER8, info.m_nAnisoDirTexture, -1 );
			}
